...
function init() {
  ...
  //   return;
  // }
  const moduleSettings = modulesSettings?.value?.find(
    (el) => el.moduleId === MODULE_KEYS.ID
  );
  ...
  const [isEnabled, id, version] = [
    ...
    Number(
      getSettingsValue(moduleSettings?.settings, MODULE_KEYS.SNIPPET_VERSION)
    ),
  ];
...
function getSettingsValue(
  settings: { name: string; value?: unknown }[] | undefined,
  settingsKey: string
): unknown {
